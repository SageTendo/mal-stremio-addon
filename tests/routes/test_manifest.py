...
class TestManifestBlueprint(unittest.TestCase):
    ...
    def test_manifest(self):
        ...
        for catalog in manifest['catalogs']:
            ...
            self.assertIn(catalog['id'],
                          ['search_list', 'plan_to_watch', 'watching', 'completed', 'on_hold', 'dropped'])
            self.assertIn(catalog['name'], ['MAL', 'MAL: Plan To Watch', 'MAL: Watching', 'MAL: Completed',
                                            'MAL: On Hold',
                                            'MAL: Dropped'])
