...
class TestMyAnimeListAPI(unittest.TestCase):
    ...
    def test_get_user_details(self, mock_get):
        ...
        mock_get.assert_called_once_with(url='https://api.myanimelist.net/v1/users/@me',
    ...
    def test_get_anime_list(self, mock_get):
        ...
        mock_get.assert_called_once_with(url='https://api.myanimelist.net/v1/anime?q=Naruto',
                                         headers={'Authorization': 'Bearer valid_access_token'},
                                         timeout=TIMEOUT)
    ...
    def test_get_user_anime_list(self, mock_get):
        ...
        mock_get.assert_called_once_with(url=f'https://api.myanimelist.net/v1/users/@me/animelist?limit={QUERY_LIMIT}',
                                         headers={'Authorization': 'Bearer valid_access_token'},
                                         timeout=TIMEOUT)
    ...
    def test_update_watched_status(self, mock_put):
        ...
        anime_id = "12345"
        episode = 5
        updated_status = self.mal_api.update_watched_status(token, anime_id, episode, status="watching")

        self.assertEqual(updated_status['status'], 'watching')
        self.assertEqual(updated_status['num_watched_episodes'], 5)
        mock_put.assert_called_once_with(url='https://api.myanimelist.net/v1/anime/12345/my_list_status',
                                         headers={'Authorization': 'Bearer valid_access_token'},
